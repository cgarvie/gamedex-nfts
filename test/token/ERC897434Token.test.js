...
const DeckRepository = artifacts.require('DeckRepository.sol');
const GDXDeck = artifacts.require('GDXDeck.sol');

require('chai')
    .use(require('chai-bignumber')(BigNumber))
    .should();

contract('GDXDeck', function (accounts) {
    const name = 'GDXCard';
    const symbol = 'GDXC';
    ...
    beforeEach(async function () {
        this.deckRepo = await DeckRepository.new({ from: creator });
        this.cardRepo = await CardRepository.new({ from: creator });
        this.token = await GDXDeck.new(this.deckRepo.address, this.cardRepo.address, { 
            from: creator, gas: 4712388, gasPrice: 100000000000 
        });
        await this.cardRepo.transferOwnership(this.token.address, { from: creator })
    });

    describe('like a full ERC897434', function () {
        beforeEach(async function () {
            await this.token.issueDeck(creator, 1000000000000000000, 5, { from: creator });
        });
        ...
        describe('issue', function () {
            ...
            beforeEach(async function () {
                await this.token.issueDeck(to, 1000000000000000000, 5, { from: creator });
            });

            it('adjusts owner tokens by index', async function () {
                const deck = await this.token.deckOfIssuerByIndex(to, 0);
                deck.toNumber().should.be.equal(2);
            });

            it('adjusts all tokens list', async function () {
                const deck = await this.token.deckByIndex(1);
                newToken.toNumber().should.be.equal(2);
            });
        });
